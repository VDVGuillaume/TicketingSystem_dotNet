...
{
    public class GetUserByUsernameQueryHandler : BaseQueryHandler<GetUserByUsernameQuery, IdentityUser>
    {
        public GetUserByUsernameQueryHandler(IMediator mediator, TicketingSystemDbContext dbContext) : base(mediator, dbContext)
        {
        }

        public async override Task<IdentityUser> ExecuteCommandAsync(GetUserByUsernameQuery request, CancellationToken cancellationToken)
        {
